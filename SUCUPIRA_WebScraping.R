...
## Função que calcula o número de programas disponíveis para a área de avaliação
## fornecida, bem como o número de páginas:
...
navigate.to_pages <- function(page.count) {
  ...
  for (j in 1:page.count) {
    ...
    ## Uma vez scrapeados todos os links da j-ésima página, forçamos o selenium 
    ## retornar à página inicial do SUCUPIRA :
    ...
    ## Clickar no botão da próxima página 'j-1' vezes :
